...
##############################################################################
    ...
    for i in range(npulses):
        ...
        real_rad_interp[i,:] = sig.polyphase_interp(k_ui, ku[i,:], 
            phs.real[i,:]*win1)
        imag_rad_interp[i,:] = sig.polyphase_interp(k_ui, ku[i,:], 
            phs.imag[i,:]*win1)
    ...
    if isSort:
        for i in range(nu):
            ...
            real_polar[:,i] = sig.polyphase_interp(k_vi, ky_new[:,i], 
                real_rad_interp[:,i]*win2)
            imag_polar[:,i] = sig.polyphase_interp(k_vi, ky_new[:,i], 
                imag_rad_interp[:,i]*win2)
    else:
        for i in range(nu):
            print('cross-range interpolating for sample %i'%(i+1))
            real_polar[:,i] = sig.polyphase_interp(k_vi, ky_new[::-1,i], 
                real_rad_interp[::-1,i]*win2)
            imag_polar[:,i] = sig.polyphase_interp(k_vi, ky_new[::-1,i], 
...
def polar_format2(phs, platform, img_plane, taylor = 20):
##############################################################################
#                                                                            #
#  This is the Polar Format algorithm.  The phase history data as well as    #
#  platform and image plane dictionaries are taken as inputs.                #
...
##############################################################################
    ...
    for i in range(npulses):
        ...
        Q_real = interp1d(dr, Q[i].real, kind = 'cubic', bounds_error = False)(dr_i)
        Q_imag = interp1d(dr, Q[i].imag, kind = 'cubic', bounds_error = False)(dr_i)
        
        Q_hat = np.nan_to_num(Q_real)+1j*np.nan_to_num(Q_imag)        
